namespace sfgmk
{
		...
		: Entity(_Position), m_fThickness(_Thickness), m_Color(_Color), m_fAlpha(0.0f), m_bIsDecreasing(false), m_bBranch(_Branch), m_iDrawNumber(0)
	{
		...
		//Init
		sf::Vector2f Tangent = _Destination - sfgmk::Convert3dTo2d(_Position);
		sf::Vector2f Normal = sfgmk::Calc_UnitVector(sf::Vector2f(Tangent.y, -Tangent.y));
		float fLength = sfgmk::Calc_Norm(Tangent);

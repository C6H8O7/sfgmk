namespace sfgmk
{
	...
	bool PhysicManager::OverlapBoxes(sf::Vector2f _Box1Center, sf::Vector2f _Box1HalfSize, sf::Vector2f _Box1Ortho[3], sf::Vector2f _Box2Center, sf::Vector2f _Box2HalfSize, sf::Vector2f _Box2Ortho[3])
	{
		...
		for( int i(0); i < 2; i++ )
		{
			fRa = _Box1HalfSize.x * ABS(sfgmk::DotProduct(_Box1Ortho[0], _Box1Ortho[i]))
				+ _Box1HalfSize.y * ABS(sfgmk::DotProduct(_Box1Ortho[1], _Box1Ortho[i]));

			fRb = _Box2HalfSize.x * ABS(sfgmk::DotProduct(_Box2Ortho[0], _Box1Ortho[i]))
				+ _Box2HalfSize.y * ABS(sfgmk::DotProduct(_Box2Ortho[1], _Box1Ortho[i]));

			if( ABS(sfgmk::DotProduct(T, _Box1Ortho[i])) > fRa + fRb )
				return false;
		}
		...
		for( int i(0); i < 2; i++ )
		{
			fRa = _Box1HalfSize.x * ABS(sfgmk::DotProduct(_Box1Ortho[0], _Box2Ortho[i]))
				+ _Box1HalfSize.y * ABS(sfgmk::DotProduct(_Box1Ortho[1], _Box2Ortho[i]));

			fRb = _Box2HalfSize.x * ABS(sfgmk::DotProduct(_Box2Ortho[0], _Box2Ortho[i]))
				+ _Box2HalfSize.y * ABS(sfgmk::DotProduct(_Box2Ortho[1], _Box2Ortho[i]));

			if( ABS(sfgmk::DotProduct(T, _Box2Ortho[i])) > fRa + fRb )
				return false;
		}
		...
		for( int i(0); i < 2; i++ )
		{
			for( int j(0); j < 2; j++ )
			{
				...
				L = sfgmk::CrossProduct(_Box1Ortho[i], _Box2Ortho[j]);

				//Test
				fRa = _Box1HalfSize.x * ABS(sfgmk::DotProduct(_Box1Ortho[0], L))
					+ _Box1HalfSize.y * ABS(sfgmk::DotProduct(_Box1Ortho[1], L));

				fRb = _Box2HalfSize.x * ABS(sfgmk::DotProduct(_Box2Ortho[0], L))
					+ _Box2HalfSize.y * ABS(sfgmk::DotProduct(_Box2Ortho[1], L));

				if( ABS(sfgmk::DotProduct(T, L)) > fRa + fRb )
					return false;
			}
		}
	...
	{
		float fDistanceSquared = sfgmk::Calc_DistanceSquared(_Collider1->getWorldCenter(), _Collider2->getWorldCenter());
	...
	bool PhysicManager::collisionObbObb(ObbCollider* _Collider1, ObbCollider* _Collider2, sfgmk::Entity* _Entity1, sfgmk::Entity* _Entity2)
	{
		...
		Box1Ortho[0] = sfgmk::Calc_UnitVector(_Collider1->getRight());
		Box1Ortho[1] = sfgmk::Calc_UnitVector(_Collider1->getUp());
		...
		Box2Ortho[0] = sfgmk::Calc_UnitVector(_Collider2->getRight());
		Box2Ortho[1] = sfgmk::Calc_UnitVector(_Collider2->getUp());

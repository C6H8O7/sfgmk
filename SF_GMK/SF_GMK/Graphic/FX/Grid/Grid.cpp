namespace sfgmk
{
	Grid::Grid() : m_BloomEffect(NULL), m_bDrawMidLines(false), m_bApplyBloom(false)
	{
	}

	Grid::Grid(const sf::Vector2i& _Lines, const sf::Vector2f& _Spacing, const sf::Vector3f& _Position) : Entity(_Position),
		m_BloomEffect(NULL), m_iLineNumber(_Lines), m_fSpacing(_Spacing), m_bDrawMidLines(false), m_bApplyBloom(false)
	{
	...
	{
		m_CameraView = CAMERA->getRect(); // get rekt lel

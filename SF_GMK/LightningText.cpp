namespace sfgmk
{
		...
		: FxText(_String, _Font, _Size, _Color), m_uiPrecision(_Precision), m_uiBoltChance(_BoltChance)
	{
		...
		m_Shader.loadFromFile("data/sfgmk/shader/intensityChange.frag", sf::Shader::Fragment);
	...
	{
		sfgmk::LightningBolt* NewBolt = NULL;
		...
		for( unsigned int i(0); i < m_uiArraySize; i++ )
		{
			if( RAND(0, m_uiBoltChance) == 0 )
			{
				...
				for( unsigned int j(0); j < m_uiPrecision; j++ )
				{
					...
					fDistance = sfgmk::Calc_DistanceSquared(m_PixelArray[i], OtherPoint);
				...
				{
					NewBolt = new sfgmk::LightningBolt(sfgmk::Convert2dTo3d(m_PixelArray[i], -1.0f), NearestParticle, false, m_Color);
					NewBolt->setMax();

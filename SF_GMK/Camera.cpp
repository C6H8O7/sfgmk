namespace sfgmk
{
	...
	void Camera::freeMove(float _TimeDelta)
	{
		//Clavier / Souris
			//Déplacements
				//Gauche
				if( SFGMK_KEYBOARD_KEY(sf::Keyboard::Left) == sfgmk::KeyStates::KEY_DOWN )
					this->move(-m_Speed.x * _TimeDelta, 0);

				//Droite
				else if( SFGMK_KEYBOARD_KEY(sf::Keyboard::Right) == sfgmk::KeyStates::KEY_DOWN )
					this->move(m_Speed.x * _TimeDelta, 0);

				//Haut
				if( SFGMK_KEYBOARD_KEY(sf::Keyboard::Up) == sfgmk::KeyStates::KEY_DOWN )
					this->move(0, -m_Speed.y * _TimeDelta);

				//Bas
				else if( SFGMK_KEYBOARD_KEY(sf::Keyboard::Down) == sfgmk::KeyStates::KEY_DOWN )
					this->move(0, m_Speed.y * _TimeDelta);

			//Zoom
				//Arriere
				if( SFGMK_MOUSE.getWheelState() == -1 )
					this->zoom(2.0f);

				//Avant
				else if( SFGMK_MOUSE.getWheelState() == 1 )
					this->zoom(0.5f);
		...
		//Manette (si un joystick connecté)
			//Déplacements
				...
				{
					sf::Vector2f JoystickPow(ABS(SFGMK_JOYSTICK_GET_AXIS(0, JOYSTICK_LEFT_HORIZONTAL)), ABS(SFGMK_JOYSTICK_GET_AXIS(0, JOYSTICK_LEFT_VERTICAL)));
					...
					if( JoystickPow.x > JOYSTICK_TOLERANCE || JoystickPow.y > JOYSTICK_TOLERANCE )
					{
						...
						sf::Vector2f Result = sfgmk::Matrix22(cos(fJoystickAngle), -sin(fJoystickAngle), sin(fJoystickAngle), cos(fJoystickAngle)) * sf::Vector2f(1.0f, 0.0f);
